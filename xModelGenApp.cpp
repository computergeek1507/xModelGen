...
#ifdef __WXMSW__
        std::string initFileName = "xModelGen.windows.properties";
#endif
#ifdef __WXOSX__
        std::string initFileName = "xModelGen.mac.properties";
        std::string resourceName = wxStandardPaths::Get().GetResourcesDir().ToStdString() + "/xModelGen.mac.properties";
...
#endif
#ifdef __LINUX__
        std::string initFileName = wxStandardPaths::Get().GetInstallPrefix() + "/bin/xModelGen.linux.properties";
        if (!wxFile::Exists(initFileName)) {
            initFileName = wxStandardPaths::Get().GetInstallPrefix() + "/share/xLights/xModelGen.linux.properties";
        }
#endif
...
xModelGenFrame *topFrame = nullptr;
...
void handleCrash(void *data) {
    ...
    //if (xModelGenFrame::GetScannerManager() != nullptr)
    //{
    //    report->SetCompressedFileDirectory(xModelGenFrame::GetScannerManager()->GetShowDir());
    //}
    ...
    //if (xModelGenFrame::GetScannerManager() != nullptr)
    //{
    //    wxFileName fn(xModelGenFrame::GetScannerManager()->GetShowDir(), OutputManager::GetNetworksFileName());
    //    if (fn.Exists()) {
    //        report->AddFile(fn.GetFullPath(), OutputManager::GetNetworksFileName());
    //    }

    //    if (wxFileName(xModelGenFrame::GetScannerManager()->GetShowDir(), ScannerManager::GetScannerFile()).Exists()) {
    //        report->AddFile(wxFileName(xModelGenFrame::GetScannerManager()->GetShowDir(), ScannerManager::GetScannerFile()).GetFullPath(), ScannerManager::GetScannerFile());
    //    }
    //}

    wxString trace = wxString::Format("xModelGen version %s\n\n", GetDisplayVersionString());
    ...
    std::string filename = std::string(dir.c_str()) + "/xModelGen_l4cpp.log";
#endif
#ifdef __WXOSX__
    wxFileName home;
    home.AssignHomeDir();
    dir = home.GetFullPath();
    std::string filename = std::string(dir.c_str()) + "/Library/Logs/xModelGen_l4cpp.log";
#endif
#ifdef __LINUX__
    std::string filename = "/tmp/xModelGen_l4cpp.log";
#endif

    if (wxFile::Exists(filename))
    {
        report->AddFile(filename, "xModelGen_l4cpp.log");
    }
    //else if (wxFile::Exists(wxFileName(xModelGenFrame::GetScannerManager()->GetShowDir(), "xModelGen_l4cpp.log").GetFullPath()))
    //{
    //    report->AddFile(wxFileName(xModelGenFrame::GetScannerManager()->GetShowDir(), "xModelGen_l4cpp.log").GetFullPath(), "xModelGen_l4cpp.log");
    //}
    else if (wxFile::Exists(wxFileName(wxGetCwd(), "xModelGen_l4cpp.log").GetFullPath()))
    {
        report->AddFile(wxFileName(wxGetCwd(), "xModelGen_l4cpp.log").GetFullPath(), "xModelGen_l4cpp.log");
    }

    //if (xModelGenFrame::GetScannerManager() != nullptr)
    //{
    //    xModelGenFrame::GetScannerManager()->CheckScannerIntegrity(false);
    //}

    if (!wxThread::IsMain() && topFrame != nullptr) {
        topFrame->CallAfter(&xModelGenFrame::CreateDebugReport, report);
        wxSleep(600000);
    }
    else {
...
void xModelGenApp::WipeSettings()
{
    static log4cpp::Category &logger_base = log4cpp::Category::getInstance(std::string("log_base"));
...
int xModelGenApp::OnExit()
{
    static log4cpp::Category &logger_base = log4cpp::Category::getInstance(std::string("log_base"));
    logger_base.info("xModelGen exiting.");

    return 0;
}

bool xModelGenApp::OnInit()
{
    ...
    static log4cpp::Category &logger_base = log4cpp::Category::getInstance(std::string("log_base"));
    logger_base.info("******* OnInit: xModelGen started.");
    ...
    if ( wxsOK )
    {
    	xModelGenFrame* Frame = new xModelGenFrame(0, singleThreaded);
    	Frame->Show();
    	SetTopWindow(Frame);
    }
    //*)
...
void xModelGenApp::OnFatalException() {
    handleCrash(nullptr);
}
